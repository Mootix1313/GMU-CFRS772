...
#	5. https://pypi.org/project/python-magic-win64/
# 	6. https://pypi.org/project/python-magic-bin/0.4.14/#files
# 	7. https://github.com/tyiannak/pyAudioAnalysis/issues/109
...
def is_keylog(filename):
    ...
    keylogger = False
    chunk_size = 4096
    my_magic = magic.Magic(magic_file="./magic.mgc")
    ...
    if is_text(filename):
        try:
            ...
            # check the magic database to weed out scripts
            if (len(re.findall(total_regex, data)) > 0) \
                    and not (("Python" in my_magic.from_file(filename))
                             or ("source" in my_magic.from_file(filename))
                             or ("exec" in my_magic.from_file(filename))):
                keylogger = True
        except Exception as e:
            print("[!] Sorry, an error occurred in is_text: "
                  "\n\t" + str(e))
...
if __name__ == "__main__":
    ...
    test_dir = r"E:\GMU DFCA\CFRS 772\Lectures\Week 11 -" \
               r" Malware (keylogger)\Lab"
    test_dir_2 = r"..\files"
    test_dir_3 = r"..\clean_files"
